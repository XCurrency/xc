...
*Setup instructions for a gitian build of XCurrency using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the XCurrency
...
- [Building XCurrency](#building-xcurrency)
...
Clone the git repositories for xcurrency and gitian and then checkout the xcurrency version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the xcurrency repository
...
To build XCurrency (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the xcurrency repository.
...
```bash
    ...
    From https://github.com/xcurrency-crypto/xcurrency
...
For example:
```bash
URL=https://github.com/crowning-/xcurrency.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit xcurrency=${COMMIT} --url xcurrency=${URL} ../xcurrency/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit xcurrency=${COMMIT} --url xcurrency=${URL} ../xcurrency/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit xcurrency=${COMMIT} --url xcurrency=${URL} ../xcurrency/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/xcurrency-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/xcurrency-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/xcurrency-build.assert
```
...
[xcurrency/gitian.sigs](https://github.com/xcurrency-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
