...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "xcurrency.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "xc3d.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 14333, 15333));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in XC/kB) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in XC/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in XC/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all XCurrency specific functionality (Servicenodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-servicenodeaddr=<n>", strprintf(_("Set external address:port to get to this servicenode (example: %s)"), "128.127.106.235:14333"));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 14332, 15332));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The XCurrency Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that XCurrency is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. XCurrency Core is shutting down."));
...
#endif
    // Make sure only a single XCurrency process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. XCurrency Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("XCurrency version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of XCurrency Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart XCurrency Core to complete") << "\n";
    ...
    CServicenodeDB mndb;
    CServicenodeDB::ReadResult readResult = mndb.Read(mnodeman);
    ...
    CServicenodePaymentDB mnpayments;
    CServicenodePaymentDB::ReadResult readResult3 = mnpayments.Read(servicenodePayments);

    if (readResult3 == CServicenodePaymentDB::FileError)
        LogPrintf("Missing servicenode payment cache - mnpayments.dat, will try to recreate\n");
    else if (readResult3 != CServicenodePaymentDB::Ok) {
        LogPrintf("Error reading mnpayments.dat: ");
        if (readResult3 == CServicenodePaymentDB::IncorrectFormat)
            LogPrintf("magic is ok but data has invalid format, will try to recreate\n");
        else
            LogPrintf("file format is unknown or invalid, please fix it manually\n");
    }
    ...
    if (fServiceNode) {
        ...
        strServiceNodeAddr = GetArg("-servicenodeaddr", "");

        LogPrintf(" addr %s\n", strServiceNodeAddr.c_str());

        if (!strServiceNodeAddr.empty()) {
            CService addrTest = CService(strServiceNodeAddr);
            if (!addrTest.IsValid()) {
                return InitError("Invalid -servicenodeaddr address: " + strServiceNodeAddr);
            }
        }

        strServiceNodePrivKey = GetArg("-servicenodeprivkey", "");
        if (!strServiceNodePrivKey.empty()) {
            ...
            if (!obfuScationSigner.SetKey(strServiceNodePrivKey, errorMessage, key, pubkey)) {
                return InitError(_("Invalid servicenodeprivkey. Please see documenation."));
            }

            activeServicenode.pubKeyServicenode = pubkey;
    ...
    //lite mode disables all Servicenode and Obfuscation related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if (fServiceNode && fLiteMode) {
