...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or xcurrency.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "        ,...\n"
            "  ],\n"
            "  \"servicenode_payments\" : true|false,         (boolean) true, if servicenode payments are enabled\n"
            "  \"enforce_servicenode_payments\" : true|false  (boolean) true, if servicenode payments are enforced\n"
            "}\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "BlocknetDX is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "BlocknetDX is downloading blocks...");
...
// Release the wallet and main lock while waiting
    ...
    result.push_back(Pair("servicenode_payments", pblock->nTime > Params().StartServicenodePayments()));
    result.push_back(Pair("enforce_servicenode_payments", true));
