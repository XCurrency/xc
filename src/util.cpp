...
int nAnonymizeXCurrencyAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "xcurrency" is a composite category enabling all xcurrency-related debug output
            if (ptrCategory->count(string("xcurrency"))) {
...
#else
    const char* pszModule = "xcurrency";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "xcurrency.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty xcurrency.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override xcurrency.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "xcurrency.pid"));
